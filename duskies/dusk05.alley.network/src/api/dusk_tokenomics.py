...
# blocking call!
def get_supply_dusk():
    circulating_supply_uri = 'https://supply.dusk.network/'
    try:
        # blocking call!
        return dict(
            circulating_supply_uri = circulating_supply_uri,
            circulating_supply_dusk = float(requests.get(circulating_supply_uri).text),
...
dusk_tokenomics = dict(
    ...
    circulating_supply = get_supply_dusk(), # Available on [this page](https://supply.dusk.network/). The circulating supply reflects the initial supply minus the DUSK held by the Dusk deployer. Post-mainnet, this value will increase as additional tokens are emitted.
...
dusk_tokenomics.emission_periods_length = len(dusk_tokenomics.emission_periods)
...
def caching(timeout):
    assert timeout >= 0, str((timeout, func))
    ...
    def wrapper(func):
        ...
        async def wrapped(*args, **kwargs):
            if monotonic() > cache.valid_until:
                ...
                cache.valid_until = monotonic() + timeout
            return cache.value
        return wrapped
...
if __name__ == '__main__':
    print(f'dusk_tokenomics_json:\n{dusk_tokenomics_json}')
